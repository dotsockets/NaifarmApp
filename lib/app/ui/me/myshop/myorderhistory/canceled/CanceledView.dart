...
class CanceledView extends StatefulWidget {
  ...
  const CanceledView({Key key, this.orderType}) : super(key: key);
  @override
...
class _CanceledViewState extends State<CanceledView> with AutomaticKeepAliveClientMixin<CanceledView>{
  ...
  init() {
    if(bloc==null){
      ...
      Usermanager().getUser().then((value) => bloc.loadOrder(orderType: widget.orderType,statusId: 2,limit: 20,page: 1,token: value.token));
    }
  ...
  Widget _ProductItem({OrderItems item,int shopId, int index}) {
    return Row(
      ...
      children: [
        InkWell(
          child: Hero(
            tag: "history_cancel_${item.orderId}${item.inventoryId}${index}",
            child: Container(
          ...
          onTap: (){
            ...
            AppRoute.ProductDetail(context, productImage: "history_cancel_${item.orderId}${item.inventoryId}${index}",productItem: ProductBloc.ConvertDataToProduct(data: product));
          },
        ),
